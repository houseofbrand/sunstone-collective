...
export const Route = createFileRoute("/blog/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.post.title} | SunglassManufacturer.com Journal` },
  ...
  notFoundComponent: () => (
    <div className="container-luxe py-24 text-center">
      ...
      <Link to="/blog" className="btn-ink mt-6 inline-flex">
        Back to journal
      </Link>
    </div>
  ),
  ...
  return (
    <>
      ...
      <article className="container-luxe pt-8 pb-24 max-w-3xl">
        ...
        <div className="prose prose-neutral mt-10 space-y-6 text-foreground leading-relaxed">
          {post.body.map((para: string, i: number) =>
            para.startsWith("## ") ? (
              <h2 key={i} className="font-display text-2xl mt-8">
                {para.replace("## ", "")}
              </h2>
            ) : (
              <p key={i}>{para}</p>
